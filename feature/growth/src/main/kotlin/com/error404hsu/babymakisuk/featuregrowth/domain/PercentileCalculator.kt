...
/**
 * WHO Child Growth Standards – 簡化 LMS 查表法。
 * L = Box-Cox power, M = median, S = coefficient of variation
 * 資料來源: WHO (2006), 依月齡 0-60 月。
 * 此為 stub 資料；正式版請從 WHO 官方 CSV 替換 lmsData。
 */
object PercentileCalculator {

    data class LmsPoint(val l: Double, val m: Double, val s: Double)

    // --- Stub tables: (ageMonths -> LmsPoint)，正式版替換為完整 WHO 表 ---
    ...
    /**
     * 回傳 0~100 的百分位數值（四捨五入到整數）。
     * @param ageMonths 月齡
     * @param value 量測值 (cm 或 kg)
     */
    ...
    /**
     * 回傳供圖表繪製的參考百分位線數值（P3 / P15 / P50 / P85 / P97）。
     */
